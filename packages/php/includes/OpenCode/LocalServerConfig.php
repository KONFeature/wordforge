...
class LocalServerConfig {
	...
	/**
	 * Build the agents configuration for local mode.
	 *
	 * Local mode uses simplified prompts without wp-cli and bash access.
	 *
	 * @return array The agents configuration.
	 */
	private static function build_agents_config(): array {
		$agents = array(
			'wordpress-manager'         => array(
				...
				'prompt'      => AgentPrompts::get_wordpress_manager_prompt( true ),
				'color'       => '#3858E9',
			),
			'wordpress-content-creator' => array(
				...
				'prompt'      => AgentPrompts::get_content_creator_prompt( true ),
				'color'       => '#10B981',
			),
			'wordpress-auditor'         => array(
				...
				'prompt'      => AgentPrompts::get_auditor_prompt( true ),
				'color'       => '#F59E0B',
			),
		);
		...
		if ( class_exists( 'WooCommerce' ) ) {
			$agents['wordpress-commerce-manager'] = array(
				...
				'prompt'      => AgentPrompts::get_commerce_manager_prompt( true ),
				'color'       => '#8B5CF6',
			);
		}
	...
	/**
	 * Get the MCP configuration for local mode.
	 *
	 * Always uses remote MCP connection since local machine can't run local MCP server
	 * that connects to WordPress.
	 *
	 * @return array|null The MCP configuration or null if unavailable.
	 */
...
MARKDOWN;
	...
	/**
	 * Get the local server settings.
	 *
	 * @return array The local server settings.
	 */
