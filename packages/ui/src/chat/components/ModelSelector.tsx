...
export const ModelSelector = ({ providers, selectedModel, onSelectModel, disabled }: ModelSelectorProps) => {
  ...
  const getModelDisplayName = (): string => {
    ...
    return model ? `${provider.name} / ${model.name}` : `${provider.name} / ${selectedModel.modelID}`;
  };
        ...
        >
          <div style={{ 
            ...
            {!Array.isArray(providers) || providers.length === 0 ? (
              <div style={{ padding: '12px 16px', color: '#646970', fontSize: '12px' }}>
                ...
                return (
                  <div key={provider.id}>
                    ...
                    {models.map(([modelID, model]) => {
                      const isSelected = selectedModel?.providerID === provider.id && selectedModel?.modelID === modelID;
                      return (
                        <button
                          key={modelID}
                        ...
                        >
                          ...
                          <div style={{ fontSize: '10px', color: '#646970', marginTop: '2px' }}>
                            ...
                            {model.status !== 'active' && (
                              <span style={{ 
                                marginLeft: '4px',
                                padding: '1px 4px', 
                                background: model.status === 'deprecated' ? '#f8d7da' : '#fff3cd',
                                borderRadius: '2px',
                                fontSize: '9px',
                              }}>
                                {model.status}
