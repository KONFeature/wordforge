...
import { WordForgeSettingsConfig } from '../types';

export const SettingsApp = () => {
  const [config] = useState<WordForgeSettingsConfig | undefined>(() => window.wordforgeSettings);

  if (!config) {
    return <div style={{ padding: '20px', color: '#d63638' }}>WordForge Settings Configuration Missing</div>;
  }
  ...
  return (
    <div className="wordforge-settings-container">
      <div className="wordforge-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '20px' }}>
        <StatusCard 
          status={statusProps} 
